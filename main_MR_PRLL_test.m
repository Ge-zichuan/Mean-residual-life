...
options = {num2str(p);num2str(d);num2str(samplesize);'norm';'ma62';'LinLi21';'unif';'LinLi22W';'gamm';'0.2'};
...
if jump == 1
    ...
    bands(1,3:4) =    [20,20]; % tuning the bandwidth in estimating \Z
    bands(2,3:6) =    [30,30,40,60]; % tuning the bandwidth in estimating \lambda, denominator
    bands(3,2:7) = [50,80,30,40,50,60]; % tuning the bandwidth in estimating \lambda, numerator
    bands(4,2:6) = [30,30,40,30,60]; % tuning the bandwidth in estimating E(YX), numerator, denominator
...
%             [xall,c,s]                     = gendata_normalize(xall,'mimic');
        ...
        try
            options1 = optimoptions('fsolve','Display','off',...
                'Algorithm','trust-region-dogleg','MaxIter', 100,'MaxFunEvals',1000);%,'TolFun',1e-5,'MaxIter', 100,'MaxFunEvals',1000
            beta0 = betaLower+(rand((p-d),d)-0.5)*0.0;
            eflag = 98;
%             bandRead = dlmread(strcat('bandsManual',num2str(filenum),'.txt'));
            for ibandpos = [7:24 27]%[8 12 16 20 24]%[7:9 11:15 19:20 22:23 27]%
                tempibands = bands(ibandpos)/20:((bands(ibandpos)*5-bands(ibandpos)/20)/5):bands(ibandpos)*5;
                for iband = 1:length(tempibands)
                    bands(ibandpos) = tempibands(iband);
                    %         for ibandManual = 1:length(bandRead(:,1))
                    %             bands(7:end) = bandRead(ibandManual,:);
                    bandadjbN = bands(1,3);
                    bandadjbT = bands(1,4);
...
%                         [betaEst,~,eflag,output] = fsolve(@(x)scoreBetaMRWTrue(x,xall,wall,zdiffallN,zdiffallT,...
%                             zallN,zallT,deltaallN,deltaallT,samplesize,samplesizeN,samplesizeT,groupIndex,p,d,bands),beta0,options1);%
                        [numOfBands,numOfParas] = testBand(filenum,bands,eflag,betaEst(:),betaLower(:),ibandpos);
                    catch
                        betaEst = NaN
                    end
                end
                ...
                bands(4,2:6) = [3,1,5,4,10]; % tuning the bandwidth in estimating E(YX), numerator, denominator
%                 bands(1,3:4) =    [20,20]; % tuning the bandwidth in estimating \Z
%                 bands(2,3:6) =    [30,30,40,60]; % tuning the bandwidth in estimating \lambda, denominator
%                 bands(3,2:7) = [40,80,30,40,50,60]; % tuning the bandwidth in estimating \lambda, numerator
%                 bands(4,2:6) = [30,30,40,30,60]; % tuning the bandwidth in estimating E(YX), numerator, denominator
            end
            algorithmtest = 1;
        catch
