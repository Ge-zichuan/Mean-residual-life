function [stdVal,jaccobVal] = jaccobBeta(parasLower,xall,zdiffall,aall,aallmat,kbz,deltaall,samplesize,goodI,q,d,bands)
...
for i = 1:d
    for j = 1:(q-d)
        jaccobValAll(goodIndexAll,(i-1)*(q-d)+j) = (lambdaVec(goodIndexAll,i).*expectVec(goodIndexAll,j));
    end
end
for i = 1:paran
    for j = 1:paran
        jaccobVal(i,j) = sum(deltaall(goodIndexAll).*jaccobValAll(goodIndexAll,i).*jaccobValAll(goodIndexAll,j));
    end
end
