function mhatfinal = estMPM1T(paras1,inputt,inputg,xall,zall,deltaall,samplesize)
...
% calculate \bb\trans\x
btransx = xallord(:,1:(end-1))*parasall(1:(end-1))+xallord(:,end)/5-10; % paras1 include upper d-by-d block.
...
% end
mhatfinal = repmat(mhat,length(inputg),1).*repelem(exp(inputg),inputn);
