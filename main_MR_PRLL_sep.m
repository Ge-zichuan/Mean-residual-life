...
p = 9;
d = 1;
options = {num2str(p);num2str(d);num2str(samplesize);'mimic';'mimic';'mimicN';'mimic';'mimicT';'gamm';'0.26'};
...
if jump == 1
    ...
    bands = zeros(4,7);
    betaEstResultN = zeros(simulations,d*(p-d));
    betaEstResultT = zeros(simulations,d*(p-d));
    ...
    bands(2,3:6) = [50,50,10,60]; % tuning the bandwidth in estimating \lambda, denominator
    bands(3,2:7) = [70,52,80,10,40,40]; % tuning the bandwidth in estimating \lambda, numerator
    bands(4,2:6) = [40,40,60,50,50]; % tuning the bandwidth in estimating E(YX), numerator, denominator
    ...
    while tt(1) <= simulations
        %% data generation
        if exist(strcat('xall',num2str(filenum),'_75','.txt'))
            ...
            wall                           = gendata_w(samplesize,options{7},quantile(tall,0.99));% generate transplant time
        ...
        try
            ...
            [betaEstT,~,eflag,output] = fsolve(@(x)scoreBetaMRT(x,xall,wall,zdiffallT,...
                kbzT,deltaallT,samplesizeT,groupIndex,p,d,bands),beta0,options1);%
            [betaEstN,~,eflag,output] = fsolve(@(x)scoreBetaMRN(x,xall,wall,zdiffallN,...
                kbzN,deltaallN,samplesizeN,groupIndex,p,d,bands),beta0,options1);%
            algorithmtest = 1;
        catch
            algorithmtest = 0;
            eflag = 99;
        ...
        if (algorithmtest==0 || eflag<1)
            ...
            betaEstResultN(tt(1),:) = betaEstN(:);
            betaEstResultT(tt(1),:) = betaEstT(:);
            xallsave(tt(1),:,:) = xall;
            zallsave(tt(1),:) = zall;
            tallsave(tt(1),:) = tall;
...
%        dlmwrite('parallel_file.txt',paraIndStart+1,'delimiter','\t');
    ...
    dlmwrite(strcat('resultBetaN',num2str(filenum),'_',num2str(paraIndStart),'.txt'),betaEstResultN,'delimiter','\t');
    dlmwrite(strcat('resultBetaT',num2str(filenum),'_',num2str(paraIndStart),'.txt'),betaEstResultT,'delimiter','\t');
%     dlmwrite(strcat('resultBetaStd',num2str(filenum*10+1),'_',num2str(paraIndStart),'.txt'),betaStdResult,'delimiter','\t');
end
