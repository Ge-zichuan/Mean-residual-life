function scoreVal = scoreBetaPM1(paras1,xallN,zallN,deltaallN,samplesizeN,xallT,zallT,deltaallT,samplesizeT,transIndex,p)
...
SNAinsideT(SNAinsideT<1e-30) = min(SNAinsideT(SNAinsideT>=1e-30));
QinsideT = mdiffallT*exp(-btransxT(:,1))./denominatorT;
SNAinsideN = exp(-(deltaordN'*mdiffallN)'./denominatorN);
SNAinsideN(SNAinsideN<1e-30) = min(SNAinsideN(SNAinsideN>=1e-30));
...
%    scoreValall(isnan(scoreValall)) = 0;
